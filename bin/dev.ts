...
async function listServers() {
  ...
  console.log(
    '─'.repeat(widths.status + widths.server + widths.url + widths.pid + widths.port + 8)
  );
...
function initializeDevEnvironment() {
  ...
  const inferredServers: Record<
    string,
    { command: string; preferredPort: number; healthCheck: string }
  > = {};
  ...
  console.log('✅ Created .dev/pids.json');
...
function parseArguments() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--log-viewer' && i + 1 < args.length) {
      ...
      parsed.command = arg;
    } else if (!parsed.action && ['start', 'stop', 'restart'].includes(arg)) {
      parsed.action = arg;
...
function getLogViewerCommand(cliLogViewer) {
  return cliLogViewer || process.env.DEV_LOG_VIEWER || 'tail -f';
}

// Main CLI
const { command, serverName, action, logViewer } = parseArguments();

// Check if command is actually a server name with an action
const servers = loadServersConfig();
const isServerCommand = command && servers[command];

if (isServerCommand) {
  // Server-specific commands: npx dev <server> <action>
  const targetServer = command;
  const targetAction = action || 'start'; // Default to start
  const logViewerCommand = getLogViewerCommand(logViewer);

  switch (targetAction) {
    case 'start':
      await startServer(targetServer, logViewerCommand);
      break;

    case 'stop':
      await stopServers(targetServer);
      break;

    case 'restart':
      // Stop server and reclaim both actual and preferred ports
      await stopServers(targetServer);
      // Start fresh
      await startServer(targetServer, logViewerCommand);
      break;
    ...
    default:
      console.error(`Unknown action: ${targetAction}`);
      process.exit(1);
  }
} else {
  // Global commands
  switch (command) {
    ...
    case undefined: {
      ...
      if (!serverName) {
        ...
        await startServer(serverName, logViewerCommand);
      }
      break;
    }
    ...
    default: {
      ...
      if (running > 0) {
        console.log('Running:');
        for (const [name, data] of Object.entries(pidData)) {
          const alive = await isProcessRunning(data.pid);
          const status = alive ? 'up' : 'dead';
          const server = servers[name];
          const portInfo =
            server && data.port !== server.preferredPort
              ? `:${server.preferredPort}→${data.port}`
              : `:${data.port}`;
          console.log(`  ${status.padEnd(5)} ${name} ${portInfo}`);
        }
        console.log('');
      }

      console.log(`Usage: npx dev <server> [action]
       npx dev <command>

Server actions:
  start                start server (default)
  stop                 stop server
  restart              restart server (kills port if needed)

Commands:
  status               show running servers with health
...
Examples:
  npx dev web                 start web server
  npx dev api restart         force restart API
  npx dev status              show running servers
  npx dev doctor              check health
`);
      break;
    }
  }
